...
Supabase устанавливается отдельно от основного проекта в `/home/alpaca/supabase/` и подключается к общей сети Docker.

## Автоматическая установка (Рекомендуется)

```bash
# Установка займет ~10-15 минут (скачивание ~500MB)
cd ~/alpaca
./scripts/setup_supabase.sh
```

Скрипт автоматически:
- Клонирует Supabase в `/home/alpaca/supabase/`
- Настроит `.env` с безопасными секретами
