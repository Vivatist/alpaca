...
```env
# Database - через Docker network (supabase-db подключён к alpaca_alpaca_network)
DATABASE_URL=postgresql://postgres:YOUR_POSTGRES_PASSWORD@supabase-db:5432/postgres

# Ollama - удалённый сервер через Tailscale (с ноутбука) или локальный (на Alpaca)
OLLAMA_BASE_URL=http://100.68.201.91:11434  # Ноутбук → Alpaca через Tailscale
# OLLAMA_BASE_URL=http://localhost:11434    # На сервере Alpaca
```
...
| PostgreSQL | Docker network | База данных (`supabase-db:5432`) |
