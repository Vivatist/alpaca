...
**Замена на Prefect 2.x:**
- ✅ Весь workflow в Python коде
- ✅ Декларативное описание flows и tasks
- ✅ Автоматические retry и кэширование
- ✅ Мощный UI для мониторинга (Prefect Server/Cloud)
- ✅ Простая отладка с breakpoints
- ✅ Версионирование в Git
- ✅ Unit-тестирование flows
- ✅ Встроенная оркестрация и scheduling
...
Фоновые задачи для автоматической обработки через Prefect 2.x.

#### scheduler.py
```python
# Prefect flows для периодических задач
- File watcher flow: каждые SCAN_INTERVAL секунд
- Queue processor flow: каждые 10 секунд
- Main orchestrator: координация всех flows
- Prefect Deployments для автоматизации
```

#### file_processor.py
```python
# Prefect flows для обработки документов
- process_document_flow: полный цикл обработки одного документа
- process_queue_flow: обработка очереди файлов
- Tasks: parse_document_task, chunk_text_task, generate_embeddings_task
- Автоматические retry и кэширование через Prefect
