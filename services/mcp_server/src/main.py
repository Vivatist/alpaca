...
_repository: Optional[MCPRepository] = None
_searcher: Optional[VectorSearcher] = None


def get_repository() -> MCPRepository:
    global _repository
    if _repository is None:
        _repository = MCPRepository(settings.DATABASE_URL)
    return _repository
...
def get_searcher() -> VectorSearcher:
    ...
    if _searcher is None:
        ...
        _searcher = VectorSearcher(embedder, repository)
    return _searcher
...
async def get_document_info(file_path: str):
    ...
    logger.info(f"📄 Document info: {file_path}")
