...
Chunker = Callable[[File], List[str]]
Embedder = Callable[[Database, File, List[str]], int]
...
class IngestDocument:
    ...
    def __call__(self, file: File) -> bool:
        ...
        try:
            ...
            self.file_manager.mark_as_ok(file)
            self.logger.info(
    ...
    def __call__(self, file_info: Dict[str, Any]) -> bool:
        ...
        try:
            if file.status_sync == "deleted":
                self.file_manager.delete_file_and_chunks(file)
                return True
            if file.status_sync == "updated":
                self.file_manager.delete_chunks_only(file)
