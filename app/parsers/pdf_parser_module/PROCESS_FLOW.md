...
- **Text**: `_parse_text()`
    1. `Unstructured`/`partition_pdf` (strategy=hi_res, rus-only). Если вернулось <100
         символов – пробуем следующий метод.
    2. `MarkItDown`
...
- **Scanned**: `_parse_scanned()`
    1. Если доступна пара `pdf2image` + `pytesseract`, прогоняем страницы через локальный
         OCR (русский язык, `--psm 3`). Собираем результат по страницам, вычисляем долю
         кириллицы для логов.
    2. Если локального OCR нет – сразу `Unstructured` с `strategy='hi_res'`.
